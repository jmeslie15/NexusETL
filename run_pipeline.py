...
def execute_pipeline(tournament_url: str):
    ...
    # Steps 2-8: The state-driven cleanup crew housed inside /src
    STATE_DRIVEN_STEPS = [
        ...
        {"name": "7. Init Derived Features Schema", "script": "3_init_derived_features.py"},
        {"name": "8. Generate Derived Features", "script": "4_compute_derived_features.py"}
    ]
    
    for step in STATE_DRIVEN_STEPS:
        # CRITICAL FIX: Pass the tournament_url parameter to every downstream task
        run_script(step["name"], step["script"], args=[tournament_url])
        time.sleep(2)

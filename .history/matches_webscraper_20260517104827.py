...
# ==========================================
def scrape_individual_game(game_url, game_number, conn, tournament_name, best_of_format):
    ...
    data = {
        ...
        'best_of': best_of_format, # Passed dynamically from the summary page
        'tournament': tournament_name # Passed dynamically from the URL
    }
    ...
    try:
        ...
        if region_block:
            ...
            region_match = re.search(r'\(([A-Z]{2,4})\)', region_text)
            data['region'] = region_match.group(1) if region_match else "Unknown"
        else:
            data['region'] = "Unknown"
...
def get_series_links_from_tournament(tournament_url):
    ...
    for a in soup.find_all('a', href=True):
        # Broadened to capture structural variants using 'page-game' directly on the matchlist
        if '/game/stats/' in a['href'] and ('page-summary' in a['href'] or 'page-game' in a['href']):
            # Clean and normalize the link format back to page-summary
...
def scrape_tournament(tournament_url, conn):
    # Dynamically extract the tournament name from the URL
    raw_name = tournament_url.strip('/').split('/')[-1]
    tournament_name = unquote(raw_name) 
    print(f"Starting Scrape for Tournament: {tournament_name}")
    ...
    for series_idx, series_url in enumerate(series_links, start=1):
        ...
        # --- DYNAMICALLY DETECT BEST OF (BO1, BO3, BO5) ---
        best_of_format = 3  # Default fallback
        format_div = soup.find('div', class_='col-4 col-sm-2 text-center')
        if format_div and format_div.h1:
            bo_match = re.search(r'BO(\d+)', format_div.h1.text)
            if bo_match:
                best_of_format = int(bo_match.group(1))
        ...
        # Fallback for BO1s where there are no 'page-game' tabs on the summary page
        if not unique_game_links:
            ...
            if game_id_match:
                ...
                if not bo_match: # type: ignore # If we couldn't find the BO text earlier, assume BO1 for untabbed matches
                    best_of_format = 1

        print(f"Found {len(unique_game_links)} games in this series. Format: BO{best_of_format}")
        
        for game_number, game_url in enumerate(unique_game_links, start=1):
            scrape_individual_game(game_url, game_number, conn, tournament_name, best_of_format)
            time.sleep(1) # Polite scraping delay between games
            
        time.sleep(2) # Polite scraping delay between series
...
if __name__ == "__main__":
    ...
    # You can change this to any valid gol.gg tournament matchlist URL

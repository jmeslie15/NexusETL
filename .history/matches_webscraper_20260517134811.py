...
def backfill_match_team_ids(conn):
    ...
    try:
        sql = """
            UPDATE matches m
            SET blue_team_id = tb.team_id
            FROM teams tb
            WHERE m.blue_team = tb.team_name AND m.blue_team_id IS NULL;

            UPDATE matches m
            SET red_team_id = tr.team_id
            FROM teams tr
            WHERE m.red_team = tr.team_name AND m.red_team_id IS NULL;
        """
        cursor.execute(sql)
        conn.commit()
...
if __name__ == "__main__":
    ...
    # Change this to any valid gol.gg tournament matchlist URL

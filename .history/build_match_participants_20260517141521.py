...
def scrape_match_participants(match_id, game_url, duration_sec, blue_team_id, red_team_id, conn):
    ...
    for idx, th in enumerate(headers):
        ...
        elif 'role' in header_text: col_map['role'] = idx
    ...
    for row_idx, row in enumerate(rows[:10]):
        ...
        # Determine side based on standard gol.gg row ordering (0-4 Blue, 5-9 Red)
        side = 'blue' if row_idx < 5 else 'red'
        team_id = blue_team_id if side == 'blue' else red_team_id

        # Extract Champion from the image alt tag in the Player column
        player_td = cols[col_map.get('player', 0)]
        champ_img = player_td.find('img', class_=re.compile('champion'))
        ...
        player_data = {
            ...
            'side': side,
            'champion': champion,
            'role': cols[col_map.get('role', 1)].text.strip() if 'role' in col_map else 'Unknown',
    ...
    for p in parsed_players:
        ...
        # Lookup player_id from our players table
        cursor.execute("SELECT player_id FROM players WHERE player_name = %s", (p['player_name'],))

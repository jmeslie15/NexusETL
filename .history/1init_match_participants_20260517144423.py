...
def scrape_match_participants(match_id, game_url, duration_sec, blue_team_id, red_team_id, conn):
    ...
    # NEW LOGIC: Look at all tables and find the one that actually holds the player stats
    main_table = None
    col_map = {}
    ...
    for table in fs_soup.find_all('table'):
        ...
        # Lock onto the table only if it has these signature stat columns
        if 'damage' in header_texts and 'gold' in header_texts:
            main_table = table
            for idx, text in enumerate(header_texts):
                # Clean up the text mapping to ensure perfect hits
                if 'kills' == text or 'k' == text: col_map['kills'] = idx
                elif 'deaths' == text or 'd' == text: col_map['deaths'] = idx
                elif 'assists' == text or 'a' == text: col_map['assists'] = idx
                elif 'cs' in text: col_map['cs'] = idx
                elif 'gold' in text: col_map['gold'] = idx
                elif 'damage' in text: col_map['damage'] = idx
                elif 'wards placed' in text or 'placed' in text: col_map['wards_placed'] = idx
                elif 'wards destroyed' in text or 'cleared' in text or 'killed' in text: col_map['wards_killed'] = idx
            break

    if not main_table:
        print("    [Error] Could not find the massive player stats table on fullstats page.")
        return
    ...
    # Helper to safely grab columns without crashing if a row is short
    def get_stat(cols_list, stat_name):
        if stat_name in col_map and col_map[stat_name] < len(cols_list):
            return safe_int(cols_list[col_map[stat_name]].text)
        return 0
    ...
    for row_idx, row in enumerate(rows[:10]):
        ...
        player_data = {
            ...
            # Use the dynamic col_map to safely extract exactly what we need
            'kills': get_stat(cols, 'kills'),
            'deaths': get_stat(cols, 'deaths'),
            'assists': get_stat(cols, 'assists'),
            'cs': get_stat(cols, 'cs'),
            'gold': get_stat(cols, 'gold'),
            'damage': get_stat(cols, 'damage'),
            'wards_placed': get_stat(cols, 'wards_placed'),
            'wards_killed': get_stat(cols, 'wards_killed')
        }
